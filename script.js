...
function renderRecords() {
  ...
  document.querySelector("#record-month-summary").textContent = `${monthRecords.length}日分の絵日記`;
  document.querySelector("#record-list-title").textContent = `${recordMonthDate.getMonth() + 1}月のページ`;
  clearFilterButton.hidden = !recordFilterTag;
  filterStatus.textContent = recordFilterTag
    ? `#${recordFilterTag} の記録：${records.length}日`
    : `この月に残した記録：${monthRecords.length}日`;
  ...
  for (let cellIndex = 0; cellIndex < totalCells; cellIndex += 1) {
    ...
    if (record) {
      ...
      art.className = "pixel-grid calendar-day-art";
  ...
  empty.querySelector("p").innerHTML = recordFilterTag
    ? `この月には #${recordFilterTag} の記録がありません。`
    : monthRecords.length
      ? "この月のページはありません。"
      : "この月は、まだまっさらです。<br>81マスの一日を残してみましょう。";
...
function renderFriends() {
  ...
  friends.forEach((sample, friendIndex) => {
    ...
    partnerLabel.textContent = "DIARY PARTNER";
    ...
    partnerNumber.textContent = `NO. ${String(friendIndex + 1).padStart(2, "0")}`;
    top.append(avatarFrame, identity, partnerNumber);
    ...
    worksCount.textContent = `${works.length} PAGES`;
